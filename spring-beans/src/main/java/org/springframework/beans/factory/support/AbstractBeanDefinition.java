...
/**
 * AbstractBeanDefinition是抽象类，它是众多子类的父类。前两篇文章讲的BeanDefinition属性值大都保存在AbstractBeanDefinition，
 * 并实现了子BeanDefinition通用方法。同时它也继承了BeanMetadataAttributeAccessor完成attribute和source的操作。
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	 ...
	 * 将不考虑该bean，即它不会被考虑作为其他bean的自动装配的候选者，
	 * 但是该bean本身还是可以使用自动装配注入其他bean的
	 */
	private boolean autowireCandidate = true;
	/**
	 * 自动装配时出现多个bean候选者时，将作为首选者，对应的bean属性Primary
	 */
